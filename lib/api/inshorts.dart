...
class InShorts {
  static String getNewsType(NewsType newsType) {
    switch (newsType) {
      case NewsType.all_news:
        return 'all_news';
      case NewsType.trending:
        return 'trending';
      case NewsType.top_stories:
        return 'top_stories';
  ...
  static String getNewsEndPoint({required NewsType newsType, Language language = Language.en, int limit = 10}) {
    if (newsType == NewsType.all_news || newsType == NewsType.trending || newsType == NewsType.top_stories) {
  ...
  static Future<Data> getNews({required NewsType newsType, Language language = Language.en, int limit = 10}) async {
    final http.Response response =
        await http.get(Uri.parse(getNewsEndPoint(newsType: newsType, language: language, limit: limit)));
